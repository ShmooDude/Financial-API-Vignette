...
This function returns detailed information about a particular ticker.

```{r}
getTickerDetails <- function(search) {
  # Retrieve the proper ticker for the search query
  ticker <- getTickers(search)
  
  # Retrieve the detailed information on the ticker
  content <- retrieveContent(path = paste0("/v3/reference/tickers/", ticker))
}
```

### `getAggregates`

Takes a particular ticker retrieves the open/close information for a number of days specified from the current date.
...
getAggregates <- function(search, multiplier = 1, 
  ...
  content <- retrieveContent(path = paste("/v2/aggs/ticker",
                               ticker, "range", multiplier, timespan, 
                               from, to,
                               sep = "/"))
}
test <- getAggregates("TSLA", from = "2020-07-22", to = "2021-07-22")
as.Date(as.POSIXct(test$results$t/1000, origin = "1970-01-01") )
```
